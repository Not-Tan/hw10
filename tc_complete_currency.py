...
NAME_TO_ISO = {"baht": "THB", "dollar": "USD", "euro": "EUR", "yen": "JPY", "bahts": "THB", "dollars": "USD", "euros": "EUR", "yens": "JPY"}
...
class CurrencyTools:
    ...
    def convert(self, amount: float, base: str, quote: str) -> Dict[str, Any]:
        ...
        rate_pair = f"{base}->{quote}"
        rate = RATE_TABLE.get(rate_pair)
        if rate is None:
            print({"error": f"No rate for {base}->{quote}"})
            return {"error": f"No rate for {base}->{quote}"}
        # print("FINAL:", getattr(msg, "rate_dict", None) or msg.get("rate_dict"))
        rate_dict: Dict[str, Any] = {"amount": amount, "base": base, "quote": quote, "rate": rate, "converted": amount * rate}
        print("Final JSON summary:", rate_dict)
        return rate_dict
...
class ToolExecutor:
    ...
    def run(self, user_text: str, model: str = MODEL, max_turns: int = 15):
